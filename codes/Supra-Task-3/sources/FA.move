module my_addrx::FAA {
    ...
    use supra_framework::object::{Self, Object, DeleteRef, TransferRef as Trf};
    use supra_framework::primary_fungible_store;
    use supra_framework::fungible_asset::{Self, MintRef, BurnRef, TransferRef, Metadata};
    ...
    fun mint(user: address, amount: u64, expiry: u64) acquires System, TokenStore {
        ...
        if (!simple_map::contains_key(&store.token_object, &user)) {
            simple_map::add(&mut store.token_object, user, vector::empty());
        };

        let user_obj_store = simple_map::borrow_mut(&mut store.token_object, &user);

        vector::push_back(user_obj_store, object_address);
    ...
    fun claim(user: address, amount: u64) acquires System, TokenObject, TokenStore {
        ...
        assert!(simple_map::contains_key(&store.token_object, &user), EUSER_NOT_FOUND);

        let user_amount_objs = simple_map::borrow_mut(&mut store.token_object, &user);

        let counter = 0;
        let claimble = amount;
        let len = vector::length(user_amount_objs);
        let total_amount = 0;

        while (counter < len && claimble > 0) {
            let object_address = vector::borrow_mut(user_amount_objs, counter);
            let obj = object::address_to_object(*object_address);

            if (check_object_expiery(*object_address)) {
                destroy_obj(obj, object_address);
                vector::remove(user_amount_objs, counter);
            } else {
                ...
                total_amount = total_amount + send_amount;
                claimble = claimble - send_amount;
            };
            counter = counter + 1;
        };
    ...
    fun transfer(user_from: address, amount: u64) acquires System, TokenStore, TokenObject {
        ...
        assert!(
            simple_map::contains_key(&store.token_object, &user_from), EUSER_NOT_FOUND
        );

        if (!simple_map::contains_key(&store.token_object, &@my_addrx)) {
            simple_map::add(&mut store.token_object, @my_addrx, vector::empty());
        };

        let counter = 0;
        let claimble = amount;
        let user_amount_objs = simple_map::borrow_mut(
            &mut store.token_object, &user_from
        );

        let len = vector::length(user_amount_objs);
        let store_addr = vector::empty<address>();
        let total_amount = 0;
        ...
        while (counter < len && claimble > 0) {
            ...
            if (check_object_expiery(*object_address)) {
                ...
                let addr = vector::remove(user_amount_objs, counter);

                vector::push_back(&mut store_addr, addr);
        ...
        vector::for_each(
            ...
            |addr| {
                vector::push_back(
                    simple_map::borrow_mut(&mut store.token_object, &@my_addrx), addr
                );
            }
        );
    ...
    public fun balance_of(account: address): u64 acquires System, TokenStore {
        ...
        assert!(
            simple_map::contains_key(&store.token_object, &account), EUSER_NOT_FOUND
        );

        let user_amount_objs = simple_map::borrow_mut(&mut store.token_object, &account);
    ...
    #[test(supra_framework = @supra_framework, admin = @my_addrx)]
    public fun test_flow(
        ...
        let alice = supra_framework::account::create_account_for_test(@0x2);
